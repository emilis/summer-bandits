...
WebMidi.addListener( 'portschanged', () => {
  ...
  midiInputs.value = WebMidi.inputs.filter( input =>
    ! input.name.match( /WebMIDI output:Output connection/ )
  );
  midiOutputs.value = WebMidi.outputs.filter( output =>
    ! output.name.match( /WebMIDI input:Input connection/ )
  );
});
