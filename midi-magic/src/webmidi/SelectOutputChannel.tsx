...
export function SelectOutputChannel({
  ...
  const onChangeOutput = (evt: JSX.TargetedEvent<HTMLSelectElement>) => {
    ...
    forSignal.value =
      output?.channels[forSignal.value?.number || channel] || null;
  };
  ...
  return (
    <div>
      <select onChange={onChangeOutput}>
        <option value={undefined}>- NO OUTPUT -</option>
        {midiOutputs.value.map((output) => (
          <option
            key={output.id}
            children={output.name}
            value={output.id}
            selected={output.id === forSignal.value?.output.id}
          />
        ))}
      </select>
